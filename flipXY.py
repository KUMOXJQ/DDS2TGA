...
def flip_images(input_folder, output_folder):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith('.tga') or filename.endswith('.dds'):
            ...
            if isinstance(image, np.ndarray):
                flipped_image = Image.fromarray(image).transpose(method=Image.FLIP_LEFT_RIGHT).transpose(method=Image.FLIP_TOP_BOTTOM)
            else:
...
if __name__ == '__main__':
    input_folder = 'path/output/tga'  # 输入文件夹路径
    output_folder = 'path/output/flip'  # 输出文件夹路径
